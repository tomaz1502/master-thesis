...
for lifting an adaptation of the scripts presented in Section~\ref{sec:gen-scripts} into
Lean proofs, based on the certifying transformations approach.
%
The specific manner in which we applied
this concept in our context was to develop a set of theorems and tactics matching
the rules present in cvc5's proof calculus.
A cvc5 proof is then a sequence of applications of those theorems and tactics.
...
every rule regarding CNF transformation) can be proved using classical
...
\medskip~In the rest of this chapter we will give an overview of the implementation of some of the
...
building block for proofs in Propositional Logic. The other two, \textit{permutateClause} and
\textit{factor}, are used to justify implicit steps (reordering of clauses and deleting repeated terms in a clause)
...
through one of the \texttt{sumBoundsThm} theorems. Addition is right-associative
in Lean by default, therefore, the proof term we are producing must prove
an inequality in which the additions are associating to the right.
