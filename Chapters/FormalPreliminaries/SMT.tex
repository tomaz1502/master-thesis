...
In this section we present the SMT problem. Recall that our goal is to translate proofs of unsatisfiability of SMT queries into Lean proofs. Therefore, besides presenting formally the problem and giving an intuition over how it is solved, we focus on showing how proofs are structured in a SMT solver.
...
\begin{enumerate}
  \item \textbf{Sorts}. Symbols identifying the kinds of variables that are allowed. They are defined by the following grammar:
        \begin{center}
          $ \tau := t \vert (\tau_{1}, \tau_{2}, \ldots, \tau_{n}) \rightarrow \tau \vert (\tau_{1}, \tau_{2}, \ldots, \tau_{m})$
        \end{center}
        Where the second case represents functions with arity $n$ and the third case represents predicates of arith $m$.
  \item \textbf{Terms}.
  \item \textbf{Formulas}.
\end{enumerate}
...
Specific theories instantiate those sets with particular values (e.g. EUF instantiate sorts with $\{U\}$, creates a function $= : U \rightarrow U \rightarrow bool$)
