...
\begin{enumerate}
  \item \textbf{Sorts}. Symbols identifying the kinds of variables that are allowed. They are defined by the following grammar:
        ...
        \end{center}
        Where the first case represents an atomic sort drawn from a set of predefined sorts, which we will refer to as $\mathcal{S}_{S}$, the second case represents functions with arity $n$ and the third case represents predicates of arith $m$.
  \item \textbf{Terms}. Symbols representing sorted variables, constants or function applications. Notice that function applications are only well formed when the sorts are respected. The sort annotations will be omitted when they can be inferred.
        \begin{center}
...
The first theory that we will be interested in this work is Equality and Uninterpreted Functions (EUF).

\begin{theorem}[Congruence]
Let $f$ be a function and $a$ and $b$ be two atoms. $a = b \rightarrow f(a) = f(b)$.
\end{theorem}
