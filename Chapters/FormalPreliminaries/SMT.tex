...
In order to check it, is enough to check if the leafs correspond to premisses, each resolution was applied correctly and the root corresponds to the empty clause. It is known that, for any unsatisfiable instance of the SAT problem, there exists at least one resolution tree that certificates its unsatisfiability~\cite{res_tree_complete}. This is the format produced by cvc5 and, therefore, we will use it in this thesis.

\subsection{Certificates for Congruence Closure}

Remember that in the theory of Equality and Uninterpreted Functions, we are given a set of equalities or inequalities and we must check whether they are consistent. Similarly to the previous case, if the set of atoms is satisfiable, a proof certificate could be just a valuation of the variables in the problem, which is trivially checked. If the set of atoms is unsatisfiable, then we can build a proof tree, similar to the resolution tree presented before. The only difference is that, instead of using the resolution theorem to transition between children and parent, this tree will use the axioms that equality satisfies (reflexivity, symmetry and transitivity), as well as Theorem~\ref{cong_theorem}. For instance, for the set of atoms $\{a \simeq f(f(a)), a \simeq f(f(f(a))), f(a) \not\simeq a\}$, a proof certificate of its unsatisfiability could be the following tree:
...
\subsection{Certificates for Linear Arithmetic}

The proof certificates for linear arithmetic rely on the following fact:

\begin{theorem}[Farkas' Lemma]
  Let $A$ be a matrix over real numbers with $n$ rows and $m$ columns. Let $b$ be a vector with $m$ entries of real numbers. The system $Ax = b$ has no solution with $x \ge 0$ if and only if the system $A^{T}y \ge 0$ has a solution with $b^{T}y < 0$.
\end{theorem}
